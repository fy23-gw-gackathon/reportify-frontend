  ...
  return {
    ...
    reports: {
      _reportId: (val1: string) => {
        ...
        return {
           ...
           */
          $put: (option: { body: Methods6['put']['reqBody'], config?: T | undefined }) =>
            fetch<void, BasicHeaders, Methods6['put']['status']>(prefix, prefix1, PUT, option).send().then(r => r.body),
          $path: () => `${prefix}${prefix1}`
        }
      }
    },
    users: {
      me: {
         ...
         */
        $get: (option?: { config?: T | undefined } | undefined) =>
          fetch<Methods7['get']['resBody'], BasicHeaders, Methods7['get']['status']>(prefix, PATH3, GET, option).json().then(r => r.body),
        $path: () => `${prefix}${PATH3}`
      }
    }
  }
